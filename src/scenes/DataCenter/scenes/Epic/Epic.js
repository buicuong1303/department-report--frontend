...
import { clearStateEpic } from './Epic.slice';
...
const Epic = props => {
  ...
  const status = useSelector(state => state.dataCenter.epic.status);
  const listData = useSelector(state => state.dataCenter.epic.listData);
  const pagination = useSelector(state => state.dataCenter.epic.pagination);
  const message = useSelector(state => state.dataCenter.epic.message);
  ...
  const handleEdit = (event, data) => {
    ...
    Object.keys(itemEditing).forEach(prop => {
      if (itemEditing[prop] === null || itemEditing[prop] === undefined)
        if(prop === 'updatedTime') itemEditing[prop] = null;
        else itemEditing[prop] = '';
    });
  ...
  const handleConfirmDelete = async () => {
    const actionResult = await dispatch(deleteDataEpicAction(infoDelete.id));
  ...
  const handleSubmit = async values => {
    dispatch(updateDataEpicAction({ id: recordEditing.id, data: values }));
    setOpenEdit(false);
    setRecordEditing(null);
  };
  ...
  useEffect(() => {
    return () => {
      dispatch(clearStateEpic());
    };
    // eslint-disable-next-line
  }, []);

  return (
    <AuthGuard requestPermissions={[permissionAuth.VIEW_ALL_EPIC_DATA]}>
