...
import CustomizedDialogs from '../Email/components/Dialog';
...
import { clearStateEmail } from './Email.slice';
...
const Email = props => {
  ...
  const { canUpdate, canDelete, departments } = props;
  ...
  const status = useSelector(state => state.dataCenter.email.status);
  const listData = useSelector(state => state.dataCenter.email.listData);
  const pagination = useSelector(state => state.dataCenter.email.pagination);
  const message = useSelector(state => state.dataCenter.email.message);
  ...
  const handleConfirmDelete = async () => {
    const actionResult = await dispatch(deleteDataEmailAction(infoDelete.id));
  ...
  const handleSubmit = async values => {
    dispatch(updateDataEmailAction({ id: recordEditing.id, data: values }));
    setOpenEdit(false);
    setRecordEditing(null);
  };
  ...
  useEffect(() => {
    return () => {
      dispatch(clearStateEmail());
    };
    // eslint-disable-next-line
  }, []);
  return (
    <AuthGuard requestPermissions={[permissionAuth.VIEW_ALL_EMAIL_DATA]}>
        ...
        <div className={classes.wrapper}>
          <MaterialTable
            ...
            columns={[
              ...
              {
                title: 'Sender or created',
                field: 'senderOrCreated',
                headerStyle: { whiteSpace: 'nowrap' }
              },
              {
                title: 'Recipients into line',
                field: 'recipientsIntoLine',
                headerStyle: { whiteSpace: 'nowrap' }
              },
              {
                title: 'Recipients into cc',
                field: 'recipientInCcLine',
                headerStyle: { whiteSpace: 'nowrap' }
              }
            ]}
            components={{
            ...
            data={JSON.parse(
              JSON.stringify(
                listData.map(item => ({
                  ...item,
                  sent: item.sent.toString().substr(0, 10),
                  received: item.received.toString().substr(0, 10)
                }))
              )
            )}
            icons={tableIcons}
